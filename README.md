...
- **카테고리 자동 선택**: 동일한 카테고리가 있으면 해당 카테고리 선택
- **태그 자동 등록**: 기존 글에 태그가 있으면 태그 자동 등록
- **일일 등록 제한**: 하루 최대 등록 개수를 제한하여 스팸 방지 정책 우회 (기본 15개/일)
  → config.json 파일에서 daily_limit을 변경하면 변경한 갯수만큼 발행합니다. (최대 15개/일)
- **인간적인 딜레이**: 글 발행 간 30초~1분 랜덤 대기로 자동화 감지 회피
...
- 미발행 폴더를 순서대로 탐색하여 하루 최대 50개씩 자동 등록 (기본 비공개)
- 글 발행 후 30초~1분 랜덤 대기 → 다음 글 발행 반복
- 하루 한도 도달 시 자동 종료
