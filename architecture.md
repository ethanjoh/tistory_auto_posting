...
| **`publish.py`** | 자동 업로드 프로세스를 총괄하는 핵심 구동 엔진 (일일 제한 확인, 최근 24시간 제한 확인, 백업 폴더 루프, Playwright 브라우저 제어, 에디터 본문 주입 및 캡차 예외 처리) |
| **`parser_utils.py`** | 각 숫자로 된 백업 폴더 내부의 HTML 구조와 로컬 이미지 경로를 파싱하여 프로그램이 주입할 수 있는 정형 데이터로 변환하는 파서 모듈 |
| **`db_utils.py`** | SQLite 데이터베이스(`database.db`) 연동 유틸리티 (테이블 초기화, 중복 발행 검사, 일일/24시간 발행 횟수 제한 검사, 다음 발행 가능 대기 시간 산출 및 이력 기록) |
| **`config.json`** | 블로그 정보, 일일 발행 개수 한도(`daily_limit`), 최근 24시간 발행 한도(`recent_24h_limit`), 브라우저 화면 표시 여부(`headless`), 지연 시간(`delays`) 및 UI 셀렉터 등을 커스텀 정의하는 설정 파일 |
...
3. **최근 24시간 제한 확인 (신규)**: 현재 시각 기준 최근 24시간 동안 성공적으로 발행 완료된 포스팅 수가 `config.json`의 `recent_24h_limit` (기본 50개)에 도달했는지 검사합니다. 50개 이상인 경우 다음 발행 가능 시각(초과분 중 가장 오래된 글의 발행 시간 + 24시간)과 남은 대기 시간을 계산하여 안내하고 즉시 프로그램을 종료합니다.
4. **미발행 폴더 수집**: 작업 디렉토리 내의 숫자명 폴더(예: `523`, `524` 등) 목록 중, 데이터베이스에 성공 이력이 등록되지 않은 폴더만 필터링하여 순서대로 수집합니다.
...
flowchart TD
    ...
    ChkLimit -- Yes --> Chk24hLimit{최근 24시간 발행 수 < 24시간 한도?}
    Chk24hLimit -- No --> ShowWaitTime[다음 발행 가능 시간 및 남은 시간 안내]
    ShowWaitTime --> End
    Chk24hLimit -- Yes --> FetchFolder[다음 미발행 폴더 선정]
